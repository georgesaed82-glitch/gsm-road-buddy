...
const defaultReasons = [
  ...
  { icon: Timer, title: "Hesitation", body: "Waiting when the road is clearly clear (roundabouts, T-junctions, traffic lights turning green) is marked the same as failing to give way. If it's safe, go.", rule: "Rules 170–172, 185" },
  { icon: Gauge, title: "Speed — too fast or too slow", body: "Going 15mph in a 30 for no reason is as much a fault as speeding. Drive at a speed that suits the road, weather, traffic and the limit.", rule: "Rules 124–125" },
  { icon: Compass, title: "Poor observation at junctions", body: "Look right, left, right again — and again if the view is restricted. Emerging without a full, effective look is one of the single biggest fail reasons every year.", rule: "Rules 170–172, 211" },
];
  ...
  return (
    <section id="common-fail-reasons" className="scroll-mt-24 border border-border bg-card p-5 sm:p-6">
      ...
      <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
        Six skills that appear again and again on DVSA's fault sheets. Fix these
        and you fix most of what stops learners passing.
      </p>
          ...
          return (
            <li key={r.title} className="flex gap-3 border border-border bg-background p-4">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 text-[10px] uppercase tracking-wider text-accent">{r.rule}</div>
              </div>
            </li>
          );
        })}
