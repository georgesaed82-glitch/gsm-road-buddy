...
function TheoryPage() {
  ...
  if (active) {
    return (
      <PortalShell
        eyebrow="Theory revision"
        title={theoryCategories.find((c) => c.slug === active)?.title ?? "Practice"}
      >
  ...
  return (
    <PortalShell eyebrow="DVSA Highway Code" title="Theory portal">
      ...
      <div className="mt-6">
        <OfflineDownloadButton
          ...
          urls={[
            ...
            "/road-signs",
            "/mock-tests",
            ...theoryCategories.map((c) => `/theory?category=${c.slug}`),
          ]}
        />
      </div>
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-muted-foreground">
              50 questions mixed across all 14 categories. Same 57-minute timer and 86% pass mark
              (43/50) as the real DVSA test. Correct answers and explanations are shown the moment
              you get one wrong.
            </p>
          </div>
          <Button onClick={() => setMock(true)} className="rounded-none" size="lg">
            Start mock test →
          </Button>
        </div>
      </div>
      ...
      <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
        Pick a category to revise. Each one has key points to learn first, then practice questions
        with explanations. Your progress saves automatically.
      </p>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {completed && (
                      <span
                        title="Quiz passed"
                        className="inline-flex items-center gap-1 rounded-full bg-success/15 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-success"
                      >
  ...
  if (pool.length === 0) {
    return (
      <div className="border border-border bg-card p-8 text-center">
        ...
        <Button onClick={onExit} className="mt-4 rounded-none" size="sm">
          ← Back to categories
        </Button>
      </div>
    );
  ...
  if (finished) {
    ...
    return (
      <div className="mx-auto max-w-xl border border-border bg-card p-8 text-center">
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {retakeMode ? "Retake complete" : "Quiz complete"}
        </div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          You got {score.correct} of {pool.length} correct.
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          {passed
            ? "Above the DVSA pass mark (86%). Strong work."
            : "Aim for 86% to match the DVSA pass mark."}
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          ...
          <Button onClick={onExit} variant="outline" className="rounded-none" size="sm">
            Back to categories
          </Button>
        </div>
      </div>
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_280px]">
      <div className="col-span-full border border-border bg-card p-4 sm:p-5">
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-full border ${lowTime ? "border-destructive bg-destructive/10" : "border-border bg-secondary/50"}`}
            >
            ...
            <div>
              ...
              <div
                className={`font-display text-3xl tabular-nums leading-none ${lowTime ? "text-destructive" : "text-foreground"}`}
              >
                {mm}:{ss}
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
            <span className="rounded-none border border-border px-3 py-1.5">
            ...
            return (
              <li key={i}>
                ...
                {answered && q.optionExplanations?.[i] && (
                  <p
                    className={`mt-2 pl-9 pr-2 text-xs leading-relaxed ${
                      state === "correct"
                        ? "text-success"
                        : state === "wrong"
                          ? "text-destructive"
      ...
      <aside className="border border-border bg-card p-5">
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          DVSA pace · {SECONDS_PER_Q}s per question.
        </p>
...
function MockExam({ onExit }: { onExit: () => void }) {
  ...
  if (finished) {
    ...
    const restart = () => {
      const fresh = shuffle(sampleTheoryQuestions).slice(
        0,
        Math.min(MOCK_TOTAL, sampleTheoryQuestions.length),
      );
      setPool(fresh);
    ...
    return (
      <div className="mx-auto max-w-2xl border border-border bg-card p-8">
        ...
        <div
          className={`mt-4 text-center font-display text-6xl ${passed ? "text-success" : "text-foreground"}`}
        >
          {pct}%
        </div>
        <p className="mt-2 text-center text-sm text-muted-foreground">
          You got {correctCount} of {pool.length} correct.{" "}
          {passed
            ? "Above the DVSA pass mark of 86% (43/50)."
            : "Aim for 86% (≥43/50) to match the DVSA pass mark."}
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <Button onClick={restart} className="rounded-none" size="sm">
            Take another mock →
          </Button>
          <Button onClick={onExit} variant="outline" className="rounded-none" size="sm">
            Back to theory
              ...
              return (
                <li key={t.slug} className="border border-border p-3">
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <div
                      className={`font-display text-lg tabular-nums ${noQs ? "text-muted-foreground" : topicPassed ? "text-success" : "text-destructive"}`}
                    >
                      {noQs ? "—" : `${t.pct}%`}
                    </div>
                  </div>
                  <div className="mt-1 flex items-center justify-between text-[11px] text-muted-foreground">
                    <span>
                      ...
                      {t.answered < t.total ? ` · ${t.total - t.answered} unanswered` : ""}
                    </span>
              ...
              return (
                <li key={i} className="border border-border p-4">
                  ...
                  <p className="mt-2 text-xs text-success">
                    Correct answer: {String.fromCharCode(65 + qq.correctIndex)}.{" "}
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_280px]">
      <div className="col-span-full border border-border bg-card p-4 sm:p-5">
        ...
        <div className="mt-3">
          <Progress
            value={(answers.filter((a) => a !== null).length / pool.length) * 100}
            className="h-1.5"
          />
        </div>
      </div>
        ...
        {answered && (
          <div className="mt-6 border-l-4 border-accent bg-accent/5 p-4">
            <div className={`text-sm font-medium ${correct ? "text-success" : "text-destructive"}`}>
              {correct
                ? "Correct"
                : `Not quite — the correct answer is ${String.fromCharCode(65 + q.correctIndex)}. ${q.options[q.correctIndex]}`}
            </div>
            <p className="mt-1 text-sm text-muted-foreground">{q.explanation}</p>
          </div>
        )}
      ...
      <aside className="border border-border bg-card p-5">
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          57 minutes for {pool.length} questions · pass at 86%.
        </p>
        <div className="mt-4">
          <div className="text-xs text-muted-foreground">Answered</div>
          <div className="font-display text-2xl">
            {answers.filter((a) => a !== null).length} / {pool.length}
          </div>
        </div>
        <Progress
          value={(answers.filter((a) => a !== null).length / pool.length) * 100}
      ...
      <div className="grid gap-6 p-6 lg:grid-cols-2">
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li>
              · <span className="text-foreground">Part 1:</span> 50 multiple-choice questions across
              the 14 topics. You need 43 to pass.
            </li>
            <li>
              · <span className="text-foreground">Part 2:</span> 14 hazard-perception clips. One has
              2 hazards, the rest have 1. Up to 5 points per hazard.
            </li>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li>
              · <span className="text-foreground">Days 1–4:</span> Read the key points for 3–4
              categories per day.
            </li>
            <li>
              · <span className="text-foreground">Days 5–9:</span> 50-question mock sessions. Review
              every wrong answer.
            </li>
            <li>
              · <span className="text-foreground">Days 10–12:</span> Hazard clips daily — aim for
              4/5 on each.
            </li>
            <li>
              · <span className="text-foreground">Days 13–14:</span> Two full mocks. Book the test
              when you score 47+ twice.
            </li>
          </ul>
        </div>
      </div>
      <div className="border-t border-border bg-secondary/30 px-6 py-4 text-xs text-muted-foreground">
        Stuck on a topic? WhatsApp George on 07961 585231 — happy to walk you through it before your
        next lesson.
      </div>
    </section>
  );
