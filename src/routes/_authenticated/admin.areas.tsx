...
const EMPTY: Draft = {
  id: "", slug: "", area: "", postcode: "", intro: "", routes_text: "",
  order_index: 0, enabled: true, nearbyText: "", highlightsText: "", faqsText: "",
};
  ...
  return (
    <AdminShell eyebrow="Admin" title="Areas covered">
      <div className="mb-4 flex justify-end gap-2">
        ...
        <Button onClick={addNew}><Plus className="mr-1 h-4 w-4" />Add area</Button>
      </div>
          ...
          <Card key={d.id || `new-${idx}`}>
            ...
            <CardContent className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-3">
                <div><Label>Area name</Label><Input className="mt-1" value={d.area} onChange={(e) => patch(idx, { area: e.target.value })} /></div>
                <div><Label>Slug (URL)</Label><Input className="mt-1" value={d.slug} onChange={(e) => patch(idx, { slug: e.target.value })} /></div>
                <div><Label>Postcode</Label><Input className="mt-1" value={d.postcode} onChange={(e) => patch(idx, { postcode: e.target.value })} /></div>
              </div>
              <div><Label>Nearby postcodes (comma-separated)</Label><Input className="mt-1" value={d.nearbyText} onChange={(e) => patch(idx, { nearbyText: e.target.value })} /></div>
              <div><Label>Intro paragraph</Label><Textarea rows={3} className="mt-1" value={d.intro} onChange={(e) => patch(idx, { intro: e.target.value })} /></div>
              <div><Label>Highlights (one per line)</Label><Textarea rows={4} className="mt-1" value={d.highlightsText} onChange={(e) => patch(idx, { highlightsText: e.target.value })} /></div>
              <div><Label>Routes description</Label><Textarea rows={3} className="mt-1" value={d.routes_text} onChange={(e) => patch(idx, { routes_text: e.target.value })} /></div>
              <div>
                <Label>FAQs (each block: <code>Q: …</code> newline <code>A: …</code>, blocks separated by blank lines)</Label>
                <Textarea rows={6} className="mt-1 font-mono text-xs" value={d.faqsText} onChange={(e) => patch(idx, { faqsText: e.target.value })} />
              </div>
