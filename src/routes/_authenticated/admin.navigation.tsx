...
function NavigationPage() {
  ...
  const move = (id: string, delta: number) => {
    ...
    setItems((its) =>
      its.map((x) =>
        patches.find((p) => p.id === x.id)
          ? { ...x, order_index: patches.find((p) => p.id === x.id)!.order_index }
          : x,
      ),
    );
  };
  ...
  return (
    <AdminShell eyebrow="Admin" title="Navigation menus">
      <p className="mb-4 text-sm text-muted-foreground">
        Edit the header, footer and learner-portal menus. Toggle the switch to hide a link without
        deleting it. Reorder with the arrows.
      </p>
          ...
          return (
            <Card key={v}>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => add(v)}>
              ...
              <CardContent className="space-y-3">
                ...
                {list.map((it, i) => (
                  <div
                    key={it.id}
                    className="grid gap-2 border border-border bg-background p-3 sm:grid-cols-[1fr_1.5fr_auto_auto]"
                  >
                    <div>
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex items-center gap-2">
                        <Switch
                          checked={it.enabled}
                          onCheckedChange={(v2) => update(it.id, { enabled: v2 })}
                        />
