...
function SeoPage() {
  ...
  const { data: settings = [] } = useQuery({
    ...
    queryFn: () => listSettingsFn(),
  });
  const ratingRow = settings.find((s) => s.key === "site_rating");
  const ratingValue = (ratingRow?.value ?? {}) as {
    rating?: number;
    review_count?: number;
    show?: boolean;
  };
  ...
  const saveRating = async () => {
    ...
    try {
      await upsertSettingFn({
        data: {
          key: "site_rating",
          value: {
            rating: Number(rating.rating),
            review_count: Math.round(Number(rating.review_count)),
            show: rating.show,
          },
        },
      });
  ...
  return (
    <AdminShell eyebrow="Admin" title="SEO editor">
      <p className="mb-4 text-sm text-muted-foreground">
        Override the title, description and social preview for any public page. Leave a field blank
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">
              Aggregate rating (used in JSON-LD, meta, and public rating labels)
            </span>
          </div>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="flex items-end gap-3">
            <div className="flex items-center gap-2">
              <Switch
                checked={rating.show}
                onCheckedChange={(v) => setRating({ ...rating, show: v })}
              />
              <span className="text-sm">Show publicly</span>
            </div>
            <Button onClick={saveRating} disabled={savingRating} className="ml-auto">
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
        <div>
          ...
          <Select
            value={current.path}
            onValueChange={(p) => setIdx(ROUTES.findIndex((r) => r.path === p))}
          >
            ...
            <SelectContent className="max-h-72">
              {ROUTES.map((r, i) => (
                <SelectItem key={r.path} value={r.path}>
                  {i + 1}. {r.label} — {r.path} {byRoute.has(r.path) ? "•" : ""}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="text-sm text-muted-foreground">
          {idx + 1} of {ROUTES.length} · {rows.length} overridden
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {existing && (
              <Badge className="bg-emerald-600 text-white">
                <CheckCircle2 className="mr-1 h-3 w-3" /> overridden
              </Badge>
            )}
          </div>
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() => setIdx((i) => Math.min(ROUTES.length - 1, i + 1))}
            >
        ...
        <CardContent className="space-y-4">
          <div>
            <Label>
              Page title <span className="text-xs text-muted-foreground">(under 60 chars)</span>
            </Label>
            <Input
              className="mt-1"
              maxLength={200}
          ...
          <div>
            <Label>
              Meta description{" "}
              <span className="text-xs text-muted-foreground">(under 160 chars)</span>
            </Label>
            <Textarea
          ...
          <div>
            ...
            <Input
              className="mt-1"
              value={form.canonical_override}
              onChange={(e) => setForm({ ...form, canonical_override: e.target.value })}
          ...
          <div className="flex items-center gap-3 border-t pt-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Adds a noindex tag so Google won't list this page.
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 border-t pt-4">
            ...
            {existing && (
              <Button variant="outline" onClick={removeOverride}>
                <RotateCcw className="mr-2 h-4 w-4" /> Remove override
              </Button>
            )}
          </div>
        </CardContent>
