...
function MeetingScene(t: number) {
  ...
  const phaseA = t < 0.2; // approach + scan
  const phaseB = t >= 0.2 && t < 0.42; // ease into half-in/half-out
  ...
  if (phaseA) {
    ...
    egoX = 40 + easeInOut(t / 0.2) * 190; // 40 → 230
    egoY = 218;
  } else if (phaseB) {
    // Slow down and steer HALF IN / HALF OUT of the parking gap
    const k = easeInOut((t - 0.2) / 0.22);
  ...
  let redX = 640;
  if (phaseA) {
    redX = 640 - easeInOut(t / 0.2) * 140; // 640 → 500 (still ahead)
  } else if (phaseB) {
    redX = 500 - easeInOut((t - 0.2) / 0.22) * 120; // 500 → 380
  } else if (phaseC) {
  ...
  const speed = phaseA ? 20 : phaseB ? 10 : phaseC ? 0 : phaseD ? 5 : 15;
  const speedColor = speed >= 25 ? GOOD : speed >= 12 ? WARN : BAD;
  const speedLabel = phaseC
    ? "Waiting"
    : speed >= 25
      ? "Plenty of room"
      : speed >= 12
  ...
  const banner = phaseA
    ? "1 · SCAN — narrowing + oncoming vehicle ahead"
    : phaseB
      ? "2 · HALF IN / HALF OUT — ease into position"
      : phaseC
        ? "3 · MEETING — hold. Let the oncoming car through"
        : phaseD
          ? "4 · MIRROR + SHOULDER — ease back into the lane"
...
export const meetingInSmallSpaces: Lesson = {
  ...
  why: (
    <>
      ...
      <p>
        On narrow residential streets the road belongs to whoever plans best, not whoever pushes
        hardest. Reading the road early lets you choose a position, a speed and a gap before you
        need them — so meeting oncoming traffic becomes a smooth exchange, not a stand-off.
      </p>
      <p className="font-semibold uppercase tracking-wider text-accent text-xs">
        Half in / half out — the waiting position
      </p>
      <p>
        When you need to give way, pull about halfway into the parking space and leave the rest of
        the car in the road. This tells the other driver plainly: <em>I'm waiting for you</em>.
        Fully in and you look parked. Fully out and you needlessly block the road. Half in / half
        out is the clearest possible signal without touching a single control.
      </p>
      <p className="font-semibold uppercase tracking-wider text-accent text-xs">
        Fitting into an existing gap
      </p>
      <p>
        Sometimes there's enough room to keep flowing and let the oncoming car through the parked
        vehicles first. Spot the gap early, decide whether it's genuinely big enough, and slot in
        smoothly — no sudden steering, no last-second brake.
      </p>
      <p className="font-semibold uppercase tracking-wider text-accent text-xs">
        Road position and the 1 m rule
      </p>
      <p>
        Normally you drive within your lane. When parked cars narrow your side, moving slightly
        towards the centre gives you a safer clearance from the parked vehicles. Aim for about 1
        metre from the doors: it protects you from doors opening, pedestrians stepping out, cyclists
        appearing and children emerging between cars. That metre is your reaction time.
      </p>
      <p className="font-semibold uppercase tracking-wider text-accent text-xs">
        Speed matches the space
      </p>
      <p>
        <strong>Less space = less speed.</strong> Roughly: plenty of room 30 mph · medium gap 15 mph
        · small gap 7 mph · very tight 4 mph. Treat these as guidelines only — always adjust to
        visibility, weather and hazards.
      </p>
      <p className="font-semibold uppercase tracking-wider text-accent text-xs">Hidden hazards</p>
      <p>
        A narrow street with parked cars hides children, pedestrians, cyclists overtaking queues and
        motorcyclists filtering. Slower speed means more thinking time and a much shorter stopping
        distance if any of them appears.
      </p>
    </>
  ),
  georgeExplains:
  ...
  captions: [
    {
      at: 0.0,
      label: "SCAN — spot the narrowing",
      detail: "Parked cars both sides. Oncoming vehicle in the distance.",
    },
    {
      at: 0.22,
      label: "HALF IN / HALF OUT — ease in",
      detail: "Slow, indicate, steer halfway into the parking gap.",
    },
    {
      at: 0.44,
      label: "MEETING — hold your position",
      detail: "Wait patiently. Let the oncoming car through the narrow point.",
    },
    {
      at: 0.68,
      label: "MIRROR + shoulder check",
      detail: "Check it's safe, then ease back into the lane.",
    },
    {
      at: 0.84,
      label: "CONTINUE with 1 m clearance",
      detail: "Keep about a metre from the parked doors as you drive on.",
    },
  ],
  questions: [
    {
      ...
      at: 0.18,
      prompt:
        "You've spotted an oncoming car and a narrowing ahead. Where should you position your vehicle to wait?",
      options: [
        {
          label: "Fully into an empty parking bay so you're out of the way",
          explain:
            "No — other drivers assume you've parked and will not thank you for waiting. Watch what happens next: the correct position is halfway in.",
        },
        {
          label: "Half in the parking bay, half in the road",
          correct: true,
          explain:
            "Correct. Half in / half out clearly says 'I'm waiting for you' without any ambiguity. Watch how the car eases into position, holds while the oncoming vehicle passes, then moves back out with 1 m clearance from the parked doors.",
        },
        {
          label: "Fully in the road so the oncoming car definitely sees you",
          explain:
            "No — that needlessly narrows the road and blocks the exchange. The right answer is halfway in — watch it demonstrated now.",
        },
      ],
    },
  ],
